...
local closebtex =	"Interface\\AddOns\\TheThunderUI\\media\\black-close"
...
local OnEnter = function(self)
	...
	self.drop:Show()
	self.drop:SetBackdropBorderColor(1, 1, 0)
end
...
local OnLeave = function(self)
	--if(self.quality > 1) then
		...
		self.drop:SetBackdropBorderColor(color.r, color.g, color.b)
	--else
	--	self.drop:Hide()
	--end
...
local createSlot = function(id)
	...
	icon:SetPoint("TOPLEFT", iconFrame, 2, -2)
	icon:SetPoint("BOTTOMRIGHT", iconFrame, -2, 2)
	frame.icon = icon

	local overlay = CreateFrame("Frame", nill, iconFrame)
	...
	if not frame.drop then
		local drop = CreateFrame("Frame", nil, frame)-- local drop = frame:CreateTexture(nil, "ARTWORK")
		drop:SetPoint("LEFT", icon, "RIGHT", 5, 0)
		drop:SetPoint("RIGHT", frame, "RIGHT", -3, 0)
		drop:SetPoint("TOP", frame,"TOP",0,-3)
		drop:SetPoint("BOTTOM", frame,"BOTTOM",0,3)
		drop:SetFrameLevel(frame:GetFrameLevel()-1)
		SetTemplate(drop)
		frame.drop = drop
	end
...
lb:SetHeight(9)
lb:ClearAllPoints()
lb:SetPoint("BOTTOMRIGHT", addon, "TOPRIGHT", -20, -14)
...
addon.LOOT_OPENED = function(self, event, autoloot)
	...
	if(items > 0) then
		for i=1, items do
			...
			slot.drop:SetBackdropBorderColor(color.r, color.g, color.b)
			slot.drop:Show()
