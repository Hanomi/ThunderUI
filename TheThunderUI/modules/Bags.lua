...
local function Print (x)
	DEFAULT_CHAT_FRAME:AddMessage("|cff9DC7CCBags:|r " .. x)
end
...
function Stuffing:SlotUpdate(b)
	...
	if b.Glow then
	--	b.Glow:Hide()
		b.Glow:SetBackdropBorderColor(0.3, 0.3, 0.3)
		if b.rarity then
			if b.rarity > 1 then
				b.Glow:SetBackdropBorderColor(GetItemQualityColor(b.rarity))
			--	b.Glow:Show()
			elseif b.qitem and quest_glow == 1 then
				b.Glow:SetBackdropBorderColor(1.0, 0.3, 0.3)
			--	b.Glow:Show()
			end
		end
	end
...
function Stuffing:SlotNew (bag, slot)
	...
	if 1 == 1 and not ret.Glow then
		local glow = CreateFrame("Frame", nil, ret.frame)
		glow:SetPoint("TOPLEFT", 0, 0)
		glow:SetPoint("BOTTOMRIGHT", 0, 0)
		glow:SetFrameLevel(ret.frame:GetFrameLevel()-1)
		SetTemplate(glow)
	...
	for _, b in ipairs(self.buttons) do
		if b.name then
			if not string.find (string.lower(b.name), str) then
				...
				if b.Glow then
				--	b.Glow:Show()
					b.Glow:SetBackdropBorderColor(0.8, 0.8, 0.3) -- b.Glow:SetBackdropBorderColor(0.3, 0.3, 0.3)
				end
			end
		end
	end
...
function Stuffing:CreateBagFrame(w)
	...
	if w == "Bank" then
		...
		f:SetPoint ("BOTTOMRIGHT", UIParent, "BOTTOMRIGHT", -x, y2)
	end
	-- close button
...
function Stuffing:InitBags()
	...
	detail:SetText("|cff9999ff" .. lc_stsearch)
	...
	local tooltip_show = function (self)
		...
		GameTooltip:SetText(lc_strsearch)
	end
		...
		if bag_cnt > 0 then
			...
			if not (hide_soulbag == 1 and bagType == ST_SOULBAG) then
				...
				for j = 1, bag_cnt do
					...
					if b.Glow then
						b.Glow:SetAllPoints(b.frame)
							if bagType == ST_QUIVER then
								b.Glow:SetBackdropBorderColor(0.8, 0.8, 0.2, 1)
							elseif bagType == ST_SOULBAG then
								b.Glow:SetBackdropBorderColor(0.8, 0.2, 0.2, 1)
							elseif bagType == ST_SPECIAL then
								b.Glow:SetBackdropBorderColor(0.2, 0.2, 0.8, 1)
							elseif bagType == ST_NORMAL then
								b.Glow:SetBackdropBorderColor(0.3, 0.3, 0.3, 1)
							end
					end
...
local function StuffingSlashCmd(Cmd)
	...
	elseif cmd == "purchase" then
		...
		Print("purchase - " .. L["buy new slot in bank"])
	end
end
